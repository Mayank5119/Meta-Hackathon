...
class ConstructionEnv:
    ...
    Action: {expedite_task, delay_task, reassign_resources, noop}
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, Dict[str, Any]]:
        ...
        # --- Apply agent action ---
        valid, action_msg = self._apply_action(action, reward_components, info)
        if not valid:
        ...
        if self._expedite_extra_cost > 0:
            reward_components["cost_penalty"] += COST_PENALTY_PER_1K * (
                self._expedite_extra_cost / 1_000
            )
        ...
        # --- Max steps check ---
        max_steps = self._template.max_steps
        ...
        reward_obj = Reward(
            value=total_reward,
            delay_penalty=reward_components["delay_penalty"],
            cost_penalty=reward_components["cost_penalty"],
            completion_bonus=reward_components["project_completion_bonus"],
            disruption_resolution_bonus=reward_components["disruption_resolution_bonus"],
            explanation=action_msg,
        )

        info.update({
            "reward_breakdown": reward_components,
            "reward_detail": reward_obj.dict(),
            "current_day": self._current_day,
            "projected_end_day": new_end_day,
            "original_end_day": self._original_end_day,
            "delay_days": current_delay,
            "budget_used": self._budget_used,
            "budget_total": self._template.budget,
    ...
    def state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget_used": self._budget_used,
            "tasks": {tid: self._task_dict(t) for tid, t in self._tasks.items()},
    ...
    def _do_expedite(self, action: Action, reward_components, info) -> Tuple[bool, str]:
        tid = action.task_id
        ...
        # Track for reward penalty accounting only.
        surcharge_per_day = extra * 400.0
    ...
    def _do_delay(self, action: Action, info) -> Tuple[bool, str]:
        tid = action.task_id
        if tid not in self._tasks:
            return False, f"Task {tid} not found."
        task = self._tasks[tid]
    ...
    def _do_reassign(self, action: Action, info) -> Tuple[bool, str]:
        src = action.task_id
        ...
        # Move 1 resource unit from src → dst
        src_task.resources -= 1
        dst_task.resources += 1
        info["events"].append(f"Moved 1 resource from {src} to {dst}.")

        return True, f"Reassigned 1 resource from {src} to {dst} — {dst} will finish faster."
        ...
        for d in self._disruptions:
            if not d.active and not d.resolved and d.fire_on_day <= self._current_day:
                ...
                if affected and affected.status != "completed":
                    ...
                    if affected.status != "disrupted":
                        affected.status = "disrupted"
                    apply_schedule(self._tasks)
                    info["events"].append(
                        f"DISRUPTION {d.id}: {d.description} (+{d.delay_days}d on {d.affected_task_id})"
                    )
    ...
    def _build_observation(self) -> Observation:
        ...
        for task in self._tasks.values():
            task_obs.append(TaskObservation(
                ...
                original_start_day=task.start_day,        # after apply_schedule
                current_start_day=task.start_day,
                current_end_day=task.end_day,
                status=TaskStatus(task.status) if task.status in TaskStatus._value2member_map_ else TaskStatus.PENDING,
                dependencies=task.dependencies,
                is_on_critical_path=task.is_on_critical_path,
                delay_from_original=task.delay_days,
                resources=task.resources + task.extra_resources,
        ...
        metrics = ProjectMetrics(
            ...
            budget_remaining=round(self._template.budget - self._budget_used, 2),
